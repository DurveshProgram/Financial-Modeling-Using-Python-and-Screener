...
def parse_screener_file(path):
    ...
    # ---- PRICE (historical year-end closing price -- a single row, not a
    # 'Report Date' + labelled-rows block like the sections above) ----
    price_row = _find_label_row(ws, "PRICE:")
    price_raw = _row_values(ws, price_row, 2, 11) if price_row else [None] * 10
    price_series = price_raw[start_idx:start_idx + n_years] if price_row else [None] * n_years
    ...
    # Fill any gaps in the price series (nearest available value, else current price)
    price_history = [p if isinstance(p, (int, float)) else None for p in price_series]
    for i in range(len(price_history)):
        if price_history[i] is None:
            fwd = next((v for v in price_history[i + 1:] if v is not None), None)
            back = next((v for v in reversed(price_history[:i]) if v is not None), None)
            price_history[i] = fwd or back or current_price
